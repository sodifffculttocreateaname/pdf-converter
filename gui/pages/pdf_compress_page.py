...
使用方式：
    from gui.pages import PdfCompressPage

    # 创建页面
    page = PdfCompressPage()
...
class FileItem:
    ...
    def __init__(self, file_path: Path):
        ...
        self.message: str = ""
        self.original_size: int = get_file_size(file_path)
...
class PdfCompressPage(QWidget):
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        # 转换器
        self._converter = PdfCompressConverter()
    ...
    def _init_ui(self):
        ...
        # 标题
        self._title_label = QLabel("PDF压缩")
        ...
        compress_layout.setContentsMargins(10, 15, 10, 10)

        # 压缩级别
        compress_layout.addWidget(QLabel("压缩级别:"), 0, 0, Qt.AlignmentFlag.AlignLeft)
        self._level_combo = QComboBox()
        self._level_combo.addItem("低压缩（保持质量）", "low")
        self._level_combo.addItem("中等压缩（推荐）", "medium")
        self._level_combo.addItem("高压缩（减小体积）", "high")
        self._level_combo.setCurrentIndex(1)  # 默认中等压缩
        self._level_combo.setMinimumWidth(150)
        compress_layout.addWidget(self._level_combo, 0, 1, Qt.AlignmentFlag.AlignLeft)

        # 图片质量
        compress_layout.addWidget(QLabel("图片质量:"), 1, 0, Qt.AlignmentFlag.AlignLeft)
        self._quality_spin = QSpinBox()
        self._quality_spin.setMinimum(30)
        self._quality_spin.setMaximum(100)
        self._quality_spin.setValue(85)
        self._quality_spin.setSuffix(" %")
        self._quality_spin.setMinimumWidth(100)
        compress_layout.addWidget(self._quality_spin, 1, 1, Qt.AlignmentFlag.AlignLeft)

        compress_layout.setColumnStretch(1, 1)
        settings_layout.addWidget(compress_group)

        # 压缩说明
        hint_group = QGroupBox("压缩说明")
        ...
        self._level_hint = QLabel(
            "• 低压缩：仅移除重复资源，保持原文件质量\n"
            "• 中等压缩：平衡质量和大小，适合大多数场景\n"
            "• 高压缩：激进压缩，最大程度减小文件体积"
        )
        self._level_hint.setStyleSheet("color: #666; font-size: 12px;")
        self._level_hint.setWordWrap(True)
        hint_layout.addWidget(self._level_hint)
    ...
    def _start_compression(self):
        ...
        # 检查文件列表
        if not self._files:
            QMessageBox.warning(self, "提示", "请先添加要压缩的PDF文件")
            return
        ...
        # 启动压缩
        self._worker.start()
        logger.info(f"开始PDF压缩: {len(files)} 个文件，级别: {level}")
    ...
    @Slot(list)
    def _on_compression_finished(self, results: List[ConversionResult]):
        """压缩完成处理"""
        # 更新文件状态
        ...
        message = f"压缩完成: 成功 {success_count} 个"
        if fail_count > 0:
            ...
            QMessageBox.warning(self, "压缩完成", f"{message}\n\n失败详情:\n{error_messages}")
        else:
            self._status_label.setText(message)
            QMessageBox.information(self, "压缩完成", message)

        logger.info(f"PDF压缩完成: 成功 {success_count}, 失败 {fail_count}")
