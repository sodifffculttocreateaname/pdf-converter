...
使用方式：
    from gui.pages import ImageToPdfPage
...
from config.constants import IMAGE_EXTENSIONS, ConversionStatus, format_file_size
...
class ImageToPdfPage(QWidget):
    ...
    def _init_ui(self):
        ...
        # 标题
        self._title_label = QLabel("图片转PDF")
        ...
        # 图片质量设置组
        quality_group = QGroupBox("图片质量设置")
        ...
        # DPI设置
        quality_layout.addWidget(QLabel("DPI:"), 0, 0, Qt.AlignmentFlag.AlignLeft)
        self._dpi_spin = QSpinBox()
        self._dpi_spin.setMinimum(72)
        ...
        quality_layout.addWidget(self._dpi_spin, 0, 1, Qt.AlignmentFlag.AlignLeft)

        # 压缩质量设置
        quality_layout.addWidget(QLabel("压缩质量:"), 1, 0, Qt.AlignmentFlag.AlignLeft)
        self._quality_spin = QSpinBox()
        self._quality_spin.setMinimum(1)
        ...
        quality_layout.addWidget(self._quality_spin, 1, 1, Qt.AlignmentFlag.AlignLeft)

        quality_layout.setColumnStretch(1, 1)
        settings_layout.addWidget(quality_group)

        # 选项设置组
        option_group = QGroupBox("选项设置")
        ...
        option_layout.setContentsMargins(10, 15, 10, 10)

        self._merge_check = QCheckBox("将所有图片合并为一个PDF文件")
        self._merge_check.setChecked(False)
        option_layout.addWidget(self._merge_check)
        ...
        format_label = QLabel("JPG、PNG、BMP、TIFF、GIF、WEBP")
    ...
    def _add_files(self, files: List[Path]):
        ...
        for file_path in files:
            ...
            if ext not in IMAGE_EXTENSIONS:
                logger.debug(f"忽略非图片文件: {file_path.name}")
                continue
    ...
    def _start_conversion(self):
        ...
        # 启动转换
        self._worker.start()
        logger.info(f"开始图片转PDF转换: {len(files)} 个文件")
    ...
    def _on_conversion_finished(self, results: List[ConversionResult]):
        ...
        logger.info(f"图片转PDF转换完成: 成功 {success_count}, 失败 {fail_count}")
