...
使用方式：
    from gui.pages import ExcelToPdfPage
...
from config.constants import EXCEL_EXTENSIONS, ConversionStatus, format_file_size
...
class ConversionWorker(QThread):
    ...
    def run(self):
        ...
        results = self._converter.convert(
            self._files,
            self._output_dir,
            page_orientation=self._page_orientation
        )
...
class ExcelDropArea(QWidget):
    """
    Excel文件拖拽上传区域

    专门用于Excel文件的拖拽上传组件（紧凑版本，用于左右分栏布局）。
...
class ExcelToPdfPage(QWidget):
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        # 转换器
        self._converter = ExcelToPdfConverter()
    ...
    def _init_ui(self):
        ...
        # 标题
        self._title_label = QLabel("Excel转PDF")
        ...
        page_layout.setContentsMargins(10, 15, 10, 10)

        # 页面方向
        page_layout.addWidget(QLabel("页面方向:"), 0, 0, Qt.AlignmentFlag.AlignLeft)
        self._orientation_combo = QComboBox()
        self._orientation_combo.addItem("自动（根据内容）", "auto")
        self._orientation_combo.addItem("纵向", "portrait")
        self._orientation_combo.addItem("横向", "landscape")
        self._orientation_combo.setCurrentIndex(0)
        self._orientation_combo.setMinimumWidth(150)
        page_layout.addWidget(self._orientation_combo, 0, 1, Qt.AlignmentFlag.AlignLeft)
        ...
        format_label = QLabel("XLS、XLSX (Excel 97-2003, Excel 2007+)")
        ...
        self._orientation_hint = QLabel(
            "• 自动：根据Excel内容自动判断页面方向\n"
            "• 纵向：适合列数较少的表格\n"
            "• 横向：适合列数较多的宽表格"
        )
        self._orientation_hint.setStyleSheet("color: #666; font-size: 12px;")
        self._orientation_hint.setWordWrap(True)
        hint_layout.addWidget(self._orientation_hint)
    ...
    def _on_add_folder(self):
        ...
        if folder:
            ...
            files = [
                ...
                if f.is_file() and f.suffix.lower().lstrip('.') in EXCEL_EXTENSIONS
            ]
            self._add_files(files)
    ...
    def _add_files(self, files: List[Path]):
        ...
        for file_path in files:
            ...
            if ext not in EXCEL_EXTENSIONS:
                logger.debug(f"忽略非Excel文件: {file_path.name}")
                continue
    ...
    def _start_conversion(self):
        ...
        # 检查文件列表
        if not self._files:
            QMessageBox.warning(self, "提示", "请先添加要转换的Excel文件")
            return

        # 获取设置
        output_dir = Path(self._output_dir_edit.text())
        page_orientation = self._orientation_combo.currentData()
        ...
        # 启动转换
        self._worker.start()
        logger.info(f"开始Excel转PDF转换: {len(files)} 个文件")
    ...
    def _on_conversion_finished(self, results: List[ConversionResult]):
        ...
        logger.info(f"Excel转PDF转换完成: 成功 {success_count}, 失败 {fail_count}")
