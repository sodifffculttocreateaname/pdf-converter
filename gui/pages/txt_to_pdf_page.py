...
使用方式：
    from gui.pages import TxtToPdfPage
...
class TxtDropArea(QWidget):
    """
    TXT文件拖拽上传区域

    专门用于TXT文本文件的拖拽上传组件（紧凑版本，用于左右分栏布局）。
...
class TxtToPdfPage(QWidget):
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        # 转换器
        self._converter = TxtToPdfConverter()
    ...
    def _init_ui(self):
        ...
        # 标题
        self._title_label = QLabel("TXT转PDF")
        ...
        # 字体大小
        text_layout.addWidget(QLabel("字体大小:"), 0, 0, Qt.AlignmentFlag.AlignLeft)
        ...
        text_layout.addWidget(self._font_size_spin, 0, 1, Qt.AlignmentFlag.AlignLeft)

        # 行高倍数
        text_layout.addWidget(QLabel("行高倍数:"), 1, 0, Qt.AlignmentFlag.AlignLeft)
        self._line_height_spin = QDoubleSpinBox()
        self._line_height_spin.setMinimum(1.0)
        self._line_height_spin.setMaximum(3.0)
        self._line_height_spin.setValue(1.5)
        self._line_height_spin.setSingleStep(0.1)
        self._line_height_spin.setDecimals(1)
        self._line_height_spin.setMinimumWidth(100)
        text_layout.addWidget(self._line_height_spin, 1, 1, Qt.AlignmentFlag.AlignLeft)
        ...
        format_label = QLabel("TXT 文本文件\n支持 UTF-8、GBK、GB2312 等常见编码格式")
    ...
    def _on_add_folder(self):
        ...
        if folder:
            ...
            files = [
                ...
                if f.is_file() and f.suffix.lower().lstrip('.') == 'txt'
            ]
            self._add_files(files)
    ...
    def _add_files(self, files: List[Path]):
        ...
        for file_path in files:
            ...
            if ext != 'txt':
                logger.debug(f"忽略非TXT文件: {file_path.name}")
                continue
    ...
    def _start_conversion(self):
        ...
        # 检查文件列表
        if not self._files:
            QMessageBox.warning(self, "提示", "请先添加要转换的TXT文件")
            return
        ...
        # 启动转换
        self._worker.start()
        logger.info(f"开始TXT转PDF转换: {len(files)} 个文件")
    ...
    def _on_conversion_finished(self, results: List[ConversionResult]):
        ...
        logger.info(f"TXT转PDF转换完成: 成功 {success_count}, 失败 {fail_count}")
