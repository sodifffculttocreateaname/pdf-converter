...
class ConversionWorker(QThread):
    ...
    def __init__(self, converter: PdfToWordConverter, files: List[Path],
                 output_dir: Path, start_page: int = 0, end_page: int = 0, parent=None):
    ...
    def run(self):
        ...
        if self._start_page > 0:
            kwargs['start_page'] = self._start_page
        if self._end_page > 0:
            kwargs['end_page'] = self._end_page
        results = self._converter.convert(self._files, self._output_dir, **kwargs)
...
class PdfToWordPage(QWidget):
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        self._converter = PdfToWordConverter()
        self._worker: Optional[ConversionWorker] = None
        self._init_ui()
        logger.info("PDF转Word页面初始化完成")
    ...
    def _init_ui(self):
        ...
        self._count_label = QLabel("待转换文件 (0)")
        ...
        self._add_folder_btn.setStyleSheet("QPushButton { background-color: #2196F3; color: white; border: none; border-radius: 4px; padding: 4px 10px; font-size: 12px; } QPushButton:hover { background-color: #1976D2; }")
        ...
        # 页码设置组
        pages_group = QGroupBox("页码设置")
        pages_group.setStyleSheet("QGroupBox { font-weight: bold; border: 1px solid #e0e0e0; border-radius: 6px; margin-top: 10px; padding-top: 10px; } QGroupBox::title { subcontrol-origin: margin; left: 10px; padding: 0 5px; }")
        pages_layout = QGridLayout(pages_group)
        pages_layout.setSpacing(10)
        pages_layout.setVerticalSpacing(8)

        pages_layout.addWidget(QLabel("起始页:"), 0, 0)
        self._start_page_edit = QLineEdit()
        self._start_page_edit.setPlaceholderText("1")
        self._start_page_edit.setMinimumWidth(60)
        pages_layout.addWidget(self._start_page_edit, 0, 1)

        pages_layout.addWidget(QLabel("结束页:"), 1, 0)
        self._end_page_edit = QLineEdit()
        self._end_page_edit.setPlaceholderText("最后页")
        self._end_page_edit.setMinimumWidth(60)
        pages_layout.addWidget(self._end_page_edit, 1, 1)

        pages_layout.setColumnStretch(1, 1)
    ...
    def _refresh_file_list(self):
        ...
        self._count_label.setText(f"待转换文件 ({len(self._files)})")
    ...
    def _parse_page_number(self, text: str) -> int:
        if not text.strip():
            return 0
        try:
            page = int(text.strip())
    ...
    def _start_conversion(self):
        ...
        start_page = self._parse_page_number(self._start_page_edit.text())
        end_page = self._parse_page_number(self._end_page_edit.text())
        ...
        self._worker = ConversionWorker(self._converter, files, output_dir, start_page, end_page)
