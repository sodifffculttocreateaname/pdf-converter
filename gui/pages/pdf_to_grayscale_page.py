...
使用方式：
    from gui.pages import PdfToGrayscalePage

    # 创建页面
    page = PdfToGrayscalePage()
...
from PySide6.QtWidgets import (
    ...
    QAbstractItemView, QVBoxLayout, QWidget, QButtonGroup,
...
class ConversionWorker(QThread):
    ...
    def run(self):
        ...
        results = self._converter.convert(
            self._files, self._output_dir, mode=self._mode, threshold=self._threshold
        )
...
class PdfToGrayscalePage(QWidget):
    """
    PDF转黑白功能页面
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        # 转换器
        self._converter = PdfToGrayscaleConverter()
    ...
    def _init_ui(self):
        ...
        self._grayscale_radio = QRadioButton("灰度模式 - 转换为灰度图像")
        self._grayscale_radio.setChecked(True)
        self._grayscale_radio.setStyleSheet("font-size: 12px;")
        self._mode_group.addButton(self._grayscale_radio)
        convert_layout.addWidget(self._grayscale_radio)

        self._threshold_radio = QRadioButton("二值化模式 - 转换为黑白图像")
        self._threshold_radio.setStyleSheet("font-size: 12px;")
        self._mode_group.addButton(self._threshold_radio)
        convert_layout.addWidget(self._threshold_radio)
        ...
        threshold_label.setStyleSheet("font-size: 12px; color: #666;")
        ...
        self._threshold_value_label = QLabel("128")
        self._threshold_value_label.setMinimumWidth(30)
        self._threshold_value_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self._threshold_value_label.setStyleSheet("font-size: 12px; color: #333;")
        threshold_layout.addWidget(self._threshold_value_label)

        convert_layout.addWidget(threshold_widget)

        # 连接信号
        self._threshold_radio.toggled.connect(self._on_mode_changed)
    ...
    def _on_mode_changed(self):
        """转换模式变更处理"""
        self._threshold_slider.setEnabled(self._threshold_radio.isChecked())
    ...
    def _start_conversion(self):
        ...
        mode = "threshold" if self._threshold_radio.isChecked() else "grayscale"
        ...
        # 启动转换
        self._worker.start()
        logger.info(f"开始PDF转黑白转换: {len(files)} 个文件")
    ...
    def _on_conversion_finished(self, results: List[ConversionResult]):
        ...
        if fail_count > 0:
            QMessageBox.warning(self, "转换完成", message)
        else:
            QMessageBox.information(self, "转换完成", message)

        logger.info(f"PDF转黑白转换完成: 成功 {success_count}, 失败 {fail_count}")
