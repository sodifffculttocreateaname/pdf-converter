...
使用方式：
    from gui.pages import PdfToImagePage
...
class PdfToImagePage(QWidget):
    ...
    def _init_ui(self):
        ...
        # 标题
        self._title_label = QLabel("PDF转图片")
        ...
        # 输出格式
        convert_layout.addWidget(QLabel("输出格式:"), 1, 0, Qt.AlignmentFlag.AlignLeft)
        self._format_combo = QComboBox()
        for format_name in self.OUTPUT_FORMATS.keys():
            self._format_combo.addItem(format_name)
        self._format_combo.setMinimumWidth(100)
        convert_layout.addWidget(self._format_combo, 1, 1, Qt.AlignmentFlag.AlignLeft)

        # 压缩质量
        convert_layout.addWidget(QLabel("压缩质量:"), 2, 0, Qt.AlignmentFlag.AlignLeft)
        ...
        self._quality_spin.setToolTip("仅对JPEG格式有效")
        self._quality_spin.setMinimumWidth(100)
        convert_layout.addWidget(self._quality_spin, 2, 1, Qt.AlignmentFlag.AlignLeft)
        ...
        hint_label = QLabel("压缩质量设置仅对JPEG格式有效，PNG为无损格式")
    ...
    def _start_conversion(self):
        ...
        output_format = self.OUTPUT_FORMATS[self._format_combo.currentText()]
        ...
        # 启动转换
        self._worker.start()
        logger.info(f"开始PDF转图片转换: {len(files)} 个文件")
    ...
    def _on_conversion_finished(self, results: List[ConversionResult]):
        ...
        logger.info(f"PDF转图片转换完成: 成功 {success_count}, 失败 {fail_count}")
