...
该模块提供Word文档转PDF功能的完整界面，包括：
- Word文档拖拽上传区域（支持doc、docx格式）
- 文件列表管理
- 输出目录设置
- 进度显示和取消功能
- 后台线程转换
...
使用方式：
    from gui.pages import WordToPdfPage
...
class WordDropArea(QWidget):
    """
    Word文档拖拽上传区域

    专门用于Word文档文件的拖拽上传组件（紧凑版本，用于左右分栏布局）。
    ...
    def _open_file_dialog(self):
        ...
        files, _ = QFileDialog.getOpenFileNames(
            ...
            "选择Word文件",
            "",
            "Word文档 (*.doc *.docx);;DOC文件 (*.doc);;DOCX文件 (*.docx);;所有文件 (*.*)"
        )
...
class WordToPdfPage(QWidget):
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        # 转换器
        self._converter = WordToPdfConverter()
    ...
    def _init_ui(self):
        ...
        # 标题
        self._title_label = QLabel("Word转PDF")
    ...
    def _on_add_folder(self):
        ...
        if folder:
            ...
            files = [
                ...
                if f.is_file() and f.suffix.lower().lstrip('.') in WORD_EXTENSIONS
            ]
            self._add_files(files)
    ...
    def _add_files(self, files: List[Path]):
        ...
        for file_path in files:
            ...
            if ext not in WORD_EXTENSIONS:
                logger.debug(f"忽略非Word文件: {file_path.name}")
                continue
    ...
    def _create_file_item_widget(self, file_item: FileItem, index: int) -> QWidget:
        ...
        # 文件图标（使用扩展名）
        ext_label = QLabel(file_item.extension[:4])
    ...
    def _start_conversion(self):
        ...
        # 检查文件列表
        if not self._files:
            QMessageBox.warning(self, "提示", "请先添加要转换的Word文件")
            return
        ...
        # 启动转换
        self._worker.start()
        logger.info(f"开始Word转PDF转换: {len(files)} 个文件")
    ...
    def _on_conversion_finished(self, results: List[ConversionResult]):
        ...
        logger.info(f"Word转PDF转换完成: 成功 {success_count}, 失败 {fail_count}")
