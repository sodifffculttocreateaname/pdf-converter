...
使用方式：
    from gui.pages import PdfExtractImagesPage

    # 创建页面
    page = PdfExtractImagesPage()
...
class PdfExtractImagesPage(QWidget):
    """
    PDF提取图片功能页面
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        # 转换器
        self._converter = PdfExtractImagesConverter()
    ...
    def _init_ui(self):
        ...
        self._start_btn = QPushButton("开始提取")
        ...
        """)
        self._start_btn.clicked.connect(self._start_extraction)
        ...
        # 最小图片宽度
        extract_layout.addWidget(QLabel("最小宽度:"), 0, 0, Qt.AlignmentFlag.AlignLeft)
        self._min_width_spin = QSpinBox()
        self._min_width_spin.setRange(1, 10000)
        self._min_width_spin.setValue(100)
        self._min_width_spin.setSuffix(" px")
        self._min_width_spin.setMinimumWidth(100)
        extract_layout.addWidget(self._min_width_spin, 0, 1, Qt.AlignmentFlag.AlignLeft)

        # 最小图片高度
        extract_layout.addWidget(QLabel("最小高度:"), 1, 0, Qt.AlignmentFlag.AlignLeft)
        self._min_height_spin = QSpinBox()
        self._min_height_spin.setRange(1, 10000)
        self._min_height_spin.setValue(100)
        self._min_height_spin.setSuffix(" px")
        self._min_height_spin.setMinimumWidth(100)
        extract_layout.addWidget(self._min_height_spin, 1, 1, Qt.AlignmentFlag.AlignLeft)

        # 输出格式
        extract_layout.addWidget(QLabel("输出格式:"), 2, 0, Qt.AlignmentFlag.AlignLeft)
        self._format_combo = QComboBox()
        self._format_combo.addItems(["PNG", "JPG"])
        self._format_combo.setMinimumWidth(100)
        extract_layout.addWidget(self._format_combo, 2, 1, Qt.AlignmentFlag.AlignLeft)

        extract_layout.setColumnStretch(1, 1)
        settings_layout.addWidget(extract_group)
    ...
    def _remove_file(self, index: int):
        ...
        if 0 <= index < len(self._files):
            removed_file = self._files[index]
    ...
    def _start_extraction(self):
        ...
        # 检查文件列表
        if not self._files:
            QMessageBox.warning(self, "提示", "请先添加要提取图片的PDF文件")
            return
        ...
        self._progress_bar.setValue(0)
        self._status_label.setText("正在提取...")
        ...
        # 启动提取
        self._worker.start()
        logger.info(f"开始PDF提取图片: {len(files)} 个文件")
    ...
    @Slot(list)
    def _on_extraction_finished(self, results: List[ConversionResult]):
        """提取完成处理"""
        # 更新文件状态
        ...
        message = f"提取完成: 成功 {success_count} 个"
        if fail_count > 0:
            ...
            QMessageBox.warning(self, "提取完成", f"{message}\n\n失败详情:\n{error_messages}")
        else:
            self._status_label.setText(message)
            QMessageBox.information(self, "提取完成", message)

        logger.info(f"PDF提取图片完成: 成功 {success_count}, 失败 {fail_count}")
