...
使用方式：
    from gui.pages import PdfToLongImagePage

    # 创建页面
    page = PdfToLongImagePage()
...
from PySide6.QtWidgets import (
    QComboBox, QFileDialog, QGridLayout, QGroupBox, QHBoxLayout, QLabel, QLineEdit,
...
class ConversionWorker(QThread):
    ...
    def __init__(self, converter: PdfToLongImageConverter, files: List[Path],
                 output_dir: Path, dpi: int, spacing: int, output_format: str, parent=None):
        """
        初始化工作线程

        Args:
            converter: PDF转长图转换器实例
            files: 要转换的文件列表
            output_dir: 输出目录
            dpi: DPI设置
            spacing: 页面间距
    ...
    def run(self):
        ...
        results = self._converter.convert(
            ...
            dpi=self._dpi, spacing=self._spacing, output_format=self._output_format
        )
...
class PdfToLongImagePage(QWidget):
    """
    PDF转长图功能页面
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        # 转换器
        self._converter = PdfToLongImageConverter()
    ...
    def _init_ui(self):
        ...
        # DPI设置
        convert_layout.addWidget(QLabel("DPI:"), 0, 0, Qt.AlignmentFlag.AlignLeft)
        self._dpi_spin = QSpinBox()
        self._dpi_spin.setMinimum(50)
        self._dpi_spin.setMaximum(600)
        self._dpi_spin.setValue(150)
        self._dpi_spin.setSuffix(" dpi")
        self._dpi_spin.setMinimumWidth(100)
        convert_layout.addWidget(self._dpi_spin, 0, 1, Qt.AlignmentFlag.AlignLeft)

        # 页面间距
        convert_layout.addWidget(QLabel("页面间距(px):"), 1, 0, Qt.AlignmentFlag.AlignLeft)
        self._spacing_spin = QSpinBox()
        self._spacing_spin.setMinimum(0)
        self._spacing_spin.setMaximum(100)
        self._spacing_spin.setValue(0)
        self._spacing_spin.setMinimumWidth(100)
        convert_layout.addWidget(self._spacing_spin, 1, 1, Qt.AlignmentFlag.AlignLeft)

        # 输出格式
        convert_layout.addWidget(QLabel("输出格式:"), 2, 0, Qt.AlignmentFlag.AlignLeft)
        self._format_combo = QComboBox()
        self._format_combo.addItems(["PNG", "JPG"])
        self._format_combo.setMinimumWidth(100)
        convert_layout.addWidget(self._format_combo, 2, 1, Qt.AlignmentFlag.AlignLeft)
    ...
    def _start_conversion(self):
        ...
        # 启动转换
        self._worker.start()
        logger.info(f"开始PDF转长图转换: {len(files)} 个文件")
    ...
    def _on_conversion_finished(self, results: List[ConversionResult]):
        ...
        logger.info(f"PDF转长图转换完成: 成功 {success_count}, 失败 {fail_count}")
